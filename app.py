...
with col_left:
    ...
    if uploaded_file:
        ...
        if not db_df.empty:
            ...
            default_idx = 0
            file_base_name = os.path.splitext(uploaded_file.name)[0] # 例如 '1.POSCAR' -> '1'
            
            try:
                # 尝试解析文件名为数字 ID
                file_id = int(file_base_name)
                ...
                if not match_row.empty:
                    ...
                    if matched_material_name in material_list:
                        ...
                        st.success(f"✅ **结构解析完毕！** 已自动识别到文件 ID: **{file_id}**，对应材料: **{matched_material_name}**")
            except ValueError:
                # 如果文件名不是数字，尝试全名匹配
